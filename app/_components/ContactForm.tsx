  ...
  return (
    <form className="lead-form" id="contact-form" onSubmit={submit} noValidate aria-describedby="contact-form-help">
      ...
      <div className="form-grid">
        ...
        <label className="field wide"><span>Objet *</span><select value={values.subject} onChange={(event) => update("subject", event.target.value)}><option>Diagnostic 180° offert</option><option>Digitalisation & automatisation</option><option>Organisation comptable, fiscale ou juridique</option><option>Pilotage & organisation</option><option>Accompagnement ISO</option><option>Formation professionnelle</option><option>Demande de devis</option><option>Être rappelé</option></select></label>
        <label className="field wide"><span>Votre besoin *</span><textarea id="contact-message" rows={6} placeholder="Décrivez la situation actuelle, les outils déjà utilisés et le résultat attendu — plus votre description est précise, plus le premier échange sera utile." value={values.message} onChange={(event) => update("message", event.target.value)} required aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? "contact-message-error" : undefined} />{errors.message ? <small className="field-error" id="contact-message-error">{errors.message}</small> : null}</label>
      </div>
