...
import { LOCATION, PHONE_DISPLAY, PHONE_LINK, whatsappMessages } from "@/app/_lib/site";
...
export default function ContactPage() {
  return (
    <main>
      ...
      <section className="section contact-page-section"><div className="container contact-page-layout"><div><p className="eyebrow eyebrow-dark"><span /> Contact rapide</p><h2>Choisissez le canal le plus simple.</h2><div className="contact-cards"><a href={`tel:${PHONE_LINK}`}><Phone aria-hidden="true" /><span><small>Téléphone</small><strong>{PHONE_DISPLAY}</strong></span></a><WhatsAppButton message={whatsappMessages.general} label="Écrire sur WhatsApp" /><div><MapPin aria-hidden="true" /><span><small>Localisation</small><strong>{LOCATION}</strong></span></div><div><Clock3 aria-hidden="true" /><span><small>Rendez-vous</small><strong>Sur confirmation</strong></span></div><div className="placeholder-contact"><MessageCircle aria-hidden="true" /><span><small>Email</small><strong>Adresse professionnelle à confirmer</strong></span></div></div><div className="quick-intents"><WhatsAppButton message={whatsappMessages.diagnostic} label="Demander un diagnostic" /><WhatsAppButton message={"Bonjour LIDA Solutions & Consulting,\n\nJe souhaite demander un devis pour un accompagnement."} label="Demander un devis" /><WhatsAppButton message={"Bonjour LIDA Solutions & Consulting,\n\nJe souhaite être rappelé concernant mon besoin."} label="Être rappelé" /></div></div><ContactForm /></div></section>
    </main>
  );
}
